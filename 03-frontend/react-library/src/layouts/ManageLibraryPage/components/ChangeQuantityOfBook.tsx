...
export const ChangeQuantityOfBook: React.FC<{ book: BookModel, deleteBook: any }> = (props, key) => {
    ...
    async function increaseBookQuantity() {
        const url = `${process.env.REACT_APP_API}/admin/secure/increase/bookQuantity?bookId=${props.book.id}`;
        const requestOptions = {
            method: 'PUT',
            headers: {
                Authorization: `Bearer ${authState?.accessToken?.accessToken}`,
    ...
    async function decreaseBookQuantity() {
        const url = `${process.env.REACT_APP_API}/admin/secure/decrease/bookQuantity?bookId=${props.book.id}`;
        const requestOptions = {
            method: 'PUT',
            headers: {
                Authorization: `Bearer ${authState?.accessToken?.accessToken}`,
    ...
    return (
        <div className='card mt-3 shadow p-3 mb-3 bg-body rounded'>
            <div className='row g-0'>
                ...
                <button onClick={increaseBookQuantity} className='m1 btn btn-md main-color text-white'>Add Quantity</button>
                <button onClick={decreaseBookQuantity} className='m1 btn btn-md btn-warning'>Decrease Quantity</button>
            </div>
        </div>
    );
}